...
3. **⚠️ 极其重要（破除“文件已损坏”限制）**：由于本开源软件未经苹果官方付费签名，系统可能会拦截。
   - **首选**：请双击运行 DMG 窗口内的 **`🔧双击修复打不开问题`** 工具，输入电脑密码授权修复。
   - **备选**：如果在极少数新版 macOS 中修复工具也被拦截，请阅读 DMG 内的 **`📄如果修复工具也打不开请看我.txt`** 文档，使用终端命令 `xattr -cr ...` 进行终极强制修复。
